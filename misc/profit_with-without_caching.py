...
if __name__ == "__main__":
    ...
    for _ in range(max_iters):
        ...
        for idx, num_users in enumerate(user_counts):
            ...
            # 1. Random Method
            print("Computing Profit without Caching... ")
            ...
            P_u_v_initial = np.zeros((K, num_users, num_UAVs)) # 3rd param

            B_u_v_k_initial = np.zeros((K, num_users, num_UAVs))
            q_V = np.zeros((total_contents,))
            q_U = np.zeros((total_contents,))
    ...
    plt.plot(user_counts, totalprofits_whale, 'r--s', label='After Caching', linewidth=2)
    plt.plot(user_counts, totalprofits_random, 'g-.D', label='Without Caching', linewidth=2)
    ...
    # Save and show
    filename = "profit_vs_caching_num_uavs"+str(num_users) + ".png"

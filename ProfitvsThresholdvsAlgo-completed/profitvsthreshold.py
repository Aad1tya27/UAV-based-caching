...
def content_request_probability(omega):
...
def main(q_V, q_U ,user_requests, user_pos, uav_pos, P_u_v_k, B_u_v_k, cluster_labels, K, num_users, num_UAVs, uav_density, tau_U, omega):
    pc_content_prob = content_request_probability(omega)
...
def fitness_func(position ,user_requests, user_pos, uav_pos, P_u_v_k, B_u_v_k, cluster_labels, K, num_users, num_UAVs, uav_density, tau_U, omega = 0.5):
    ...
    profit = main(qV, qU, user_requests, user_pos, uav_pos, P_u_v_k, B_u_v_k, cluster_labels, K, num_users, num_UAVs, uav_density, tau_U, omega)  # Modify your main() to accept qV/qU and return profit
    return profit
...
if __name__ == "__main__":
    ...
    num_users = 60
    K = 10
    omega = 0.5  # Fixed omega value
    threshold_values = [-1, 0, 1, 2]  # rho_UAV values
    totalprofits_random = np.zeros(len(threshold_values))
    totalprofits_whale = np.zeros(len(threshold_values))
    totalprofits_vulture = np.zeros(len(threshold_values))
    max_iters = 1
    ...
    for _ in range(max_iters):
        print(f"Running for iteration {_+1}/{max_iters}")
        for idx, rho_UAV in enumerate(threshold_values):
            print(f"Running for rho_UAV = {rho_UAV}")
            ...
            totalprofit = main(q_V, q_U, user_requests, user_pos, uav_pos, P_u_v_initial, B_u_v_k_initial, initial_cluster_labels, K, num_users, num_UAVs, uav_density, tau_U, omega)
            print(totalprofit)

            # 2. Whale Optimization Method
            print("Computing WOA Method Profit...")
            optimal_solution2 = woa_optimizer(fitness_func, user_requests, user_pos, uav_pos, P_u_v_initial, B_u_v_k_initial, initial_cluster_labels, K, num_users, num_UAVs, uav_density, tau_U, omega=omega)
            ...
            totalprofit2 = main(q_V, q_U, user_requests, user_pos, uav_pos, P_u_v_k, B_u_v_k, cluster_labels, K, num_users, num_UAVs, uav_density, tau_U, omega)
            print(totalprofit2)
            
            # 3. Vulture Method
            print("Computing AVOA Method Profit...")
            optimal_solution3 = avoa_optimizer(fitness_func, user_requests, user_pos, uav_pos, P_u_v_initial, B_u_v_k_initial, initial_cluster_labels, K, num_users, num_UAVs, uav_density, tau_U, omega=omega)
            ...
            totalprofit3 = main(q_V, q_U, user_requests, user_pos, uav_pos, P_u_v_k, B_u_v_k, cluster_labels, K, num_users, num_UAVs, uav_density, tau_U, omega)
            print(totalprofit3)
    ...
    plt.plot(threshold_values, totalprofits_random, 'b-o', label='Random Method', linewidth=2)
    plt.plot(threshold_values, totalprofits_whale, 'r--s', label='Whale Optimization', linewidth=2)
    plt.plot(threshold_values, totalprofits_vulture, 'g-.D', label='Vulture Optimization', linewidth=2)

    plt.xlabel('Threshold Value (rho_UAV)', fontsize=12)
    plt.ylabel('Total Profit', fontsize=12)
    plt.title("System Performance Comparison for Different U2D Threshold Values", fontsize=14)
    plt.xticks(threshold_values, fontsize=10)
    ...
    # Save and show
    filename = "ProfitvsThresholdvsAlgo/profit_vs_threshold.png"
    plt.savefig(filename, dpi=300)

    # Save results to a text file
    results_file = "ProfitvsThresholdvsAlgo/results.txt"
    with open(results_file, 'w') as f:
        for i, rho_UAV in enumerate(threshold_values):
            f.write(f"{rho_UAV}\t{totalprofits_random[i]}\t{totalprofits_whale[i]}\t{totalprofits_vulture[i]}\n")

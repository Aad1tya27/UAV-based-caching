...
if __name__ == "__main__":
    ...
    # Dictionary to store profit values for each M_V
    all_profits = {}
    M_V_values = list(range(100, 145, 20))
    ...
    for M_V in M_V_values:
        ...
        optimal_solution2, iterations_values_woa = woa_optimizer(fitness_func, user_requests, user_pos, uav_pos, P_u_v_initial, B_u_v_k_initial, initial_cluster_labels, K, num_users, num_UAVs, uav_density, tau_U, M_V, max_iters)
        ...
        # Store the profit values for this M_V
        all_profits[M_V] = iterations_values_woa
    
    plt.figure(figsize=(10, 6))

    # Plot a line for each M_V value
    for M_V in M_V_values:
        # Create iteration numbers for x-axis (every 5 iterations)
        step = 5
        iterations = list(range(0, len(all_profits[M_V]) + 1, step))
        
        # Filter values to plot every 5 iterations, including initial values at x=0
        values = [0]  # Start with 0 at x=0
        
        # Add the rest of the values
        values.extend([all_profits[M_V][i-1] for i in iterations[1:]])
        
        plt.plot(iterations, values, '--s', label=f'Cache Capacity = {M_V}', linewidth=2)

    plt.xlabel('Number of Iterations', fontsize=12)
    plt.ylabel('Total Profit', fontsize=12)
    plt.title("System Performance for Different Cache Capacity of UAVs", fontsize=14)
    ...
    # Save and show
    filename = "ProfitvsItersvsCache/profit_vs_iters_vs_cache.png"
    plt.savefig(filename, dpi=300)
    plt.show()
    
    # Save results to a text file
    results_file = "ProfitvsItersvsCache/results.txt"
    with open(results_file, 'w') as f:
        # Get all iterations across all M_V values
        all_iterations = set()
        for M_V in M_V_values:
            step = 5
            iterations = list(range(0, len(all_profits[M_V]) + 1, step))
        ...
        # Write each iteration on a new line with values for all M_V
